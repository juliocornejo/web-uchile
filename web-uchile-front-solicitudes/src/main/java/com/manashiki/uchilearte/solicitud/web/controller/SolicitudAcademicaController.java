    ...
    protected void doGet(javax.servlet.http.HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(solicitudAcademicaImpl.getGenerarAplicacion().getAuthenticacionContext()!=null){
				...
				logger.info("Pintando Solicitud Academica.");
				...
				logger.info("Pintando Error Solicitud Academica. Usuario Invalido");
			}
			...
			request.getRequestDispatcher("/administracion/administracion-error.jsp").forward(request, response);
			logger.info("Pintando Error Solicitud Academica. Error de los servicios interno");
		}
	}
